  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    .card::before{
      ...
      background: conic-gradient(from 0deg, rgba(122,162,247,.22), rgba(194,143,255,.22), rgba(106,227,189,.22), rgba(122,162,247,.22));
    ...
    .loader{
      ...
      padding: 26px 28px; border: 1px solid rgba(122,162,247,.28);
    ...
    /* Clicker specific small tweaks */
    .statline{ display:flex; gap:10px; justify-content:center; color:var(--muted); font-size:14px; margin-top:10px; user-select:none }
...
<body>
  ...
  <main class="grid" id="hubGrid">
    ...
    <article class="card">
      <h3>MULTIVERSAL AU CLICKER</h3>
      <p>Click to gather AU energy, unlock AUs, and climb the multiversal leaderboard.</p>
      <div class="actions">
    ...
    <article class="card">
      <h3>Coming soon</h3>
      <p>Add more games here. The shell handles loading, glow, and navigation.</p>
      <div class="actions">
      ...
      <div class="player-count" aria-label="Players online">
        <span class="player-dot" aria-hidden="true"></span>
        <span id="soonCount">Players online: 0</span>
      </div>
    </article>
  </main>
  ...
  <!-- CLICKER VIEW -->
  <section id="clickerView" style="display:none">
    <header class="topbar">
      <div class="brand">MULTIVERSAL AU CLICKER</div>
      <div class="spacer"></div>
      <button class="btn" id="backToHub">Back</button>
    </header>

    <main style="display:grid; place-items:center; padding:24px 20px 40px">
      <div class="card" style="max-width:460px; width:min(92vw,460px); text-align:center">
        <h3>AU Energy</h3>
        <div id="energy" style="font-size:36px; font-weight:800; margin:10px 0 14px">0</div>

        <button class="btn" id="clickBtn" style="font-size:18px; padding:16px 22px">Gather Energy</button>

        <div class="statline">
          <span id="perClick">Per click: 1</span>
          <span>•</span>
          <span id="perSec">+0 /s</span>
        </div>

        <div class="stack">
          <button class="btn" id="buyCursor">Buy Cursor (10)</button>
          <button class="btn" id="buyFactory">Buy Factory (100)</button>
        </div>

        <div style="display:flex; justify-content:center; gap:10px; margin-top:16px">
          <span class="pill" id="ownedCursor">Cursors: 0</span>
          <span class="pill" id="ownedFactory">Factories: 0</span>
        </div>
      </div>
    </main>

    <div class="footer" style="text-align:center">Autosaves locally. Hard reset with Ctrl+Shift+R.</div>
  </section>
  ...
  <script>
    ...
    function animateProgress(ms=1200){
      bar.style.width='0%'; label.textContent='Preparing assets…';
      let p=0;
      const step=()=>{
    ...
    // --- Player counts demo (replace with your endpoints) ---
    async function updateCounts(){
      ...
      try{
        const res3 = await fetch('/api/playercount/comingsoon', {cache:'no-store'});
        if(res3.ok){
          const data3 = await res3.json();
          document.getElementById('soonCount').textContent = 'Players online: ' + (data3.count ?? 0);
        }
      }catch(e){}
    }
    ...
    document.getElementById('play-au').addEventListener('click', async ()=>{
      ...
      clickerView.style.display='block';
    });

    document.getElementById('backToHub').addEventListener('click', ()=>{
      clickerView.style.display='none';
    ...
    // --- Minimal TicTacToe stub (optional navigation) ---
    document.getElementById('play-ttt').addEventListener('click', async ()=>{
      showLoader(true);
      await animateProgress(1000);
      showLoader(false);
      alert('Tic‑Tac‑Toe loads here.');
    });

    // --- Clicker logic with autosave ---
    const SAVE_KEY = 'au-clicker-save-v1';
    const state = {
      energy: 0,
      perClick: 1,
      cursor: 0,
    ...
    const $ownedCursor = document.getElementById('ownedCursor');
    const $ownedFactory = document.getElementById('ownedFactory');

    function pps(){ return state.cursor*0.1 + state.factory*2; }

    function render(){
      $energy.textContent = Math.floor(state.energy);
      $perClick.textContent = 'Per click: ' + state.perClick;
      $perSec.textContent = '+' + pps().toFixed(1) + ' /s';
      $ownedCursor.textContent = 'Cursors: ' + state.cursor;
      $ownedFactory.textContent = 'Factories: ' + state.factory;
    }
