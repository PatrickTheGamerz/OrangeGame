...
<style>
  ...
  *{box-sizing:border-box}
  ...
  #loaderLabel{font-size:14px; opacity:.85}
  #loaderBarWrap{
    width:100%; height:14px;
    border-radius:8px; background:#0c121c;
    overflow:hidden;
  ...
  /* CENTER: SOUL + Menu — perfectly centered */
  #stage{
    grid-column:2;
    position:relative;
    display:flex;
  ...
  /* RIGHT: AU SELECT — pinned fully right */
  #auPanel{
    grid-column:3;
...
<script>
  ...
  let lpsTick = null;
  function startLps(){
    if(lpsTick) clearInterval(lpsTick);
    lpsTick = setInterval(()=>{
      let totalLps = 0;
